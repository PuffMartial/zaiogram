...
class App {
  ...
  handleLogout(event) {
    const isLogoutButtonClickedOn = this.$logoutButton.contains(event.target);
    if (isLogoutButtonClickedOn) {
  ...
  uploadToFireBase() {
    if (files.length != 0) {
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        const storage = firebase.storage().ref(files[i].name);
//upload file
        const upload = storage.put(files[i]);
         //update progress bar
        upload.on(
          "state_changed",
    ...
    //    const upload = firebase.storage().ref(files[i].name);
    //   upload
    //     .then((snapshot) => {
